...
def read_masses():
    ...
    datastream = pkgutil.get_data(
        '.', 'data/input_masses/atomic_masses.dat')
    '''

    with open ("burnman/data/input_masses/atomic_masses.dat", "r") as myfile:
        datastream=myfile.readlines()

    datalines = [line.strip()
                 for line in datastream if line.strip()]
    lookup = dict()
